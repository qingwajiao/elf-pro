...
contract ElfMarket is Ownable, ReentrancyGuard {
    ...
    // pass  --> 有待优化
    function putNft(address _admin)public {

        uint[]memory nftList = getUserNftList(_admin);
        uint len = nftList.length;
        for (uint index =0;index < len; index ++){

            IERC721Enumerable(elfNFTContractAddress).transferFrom(_admin, address(this), nftList[index]);
    ...
    // 查询用户拥有的nft
    function checkBalance(address _userAddress)public returns(uint[] memory nftlist){
        uint  amount =  IERC721Enumerable(elfNFTContractAddress).balanceOf(_userAddress);
        ...
        for (uint index = 0;index< amount;index++){
            tempUserNFTs.push(IERC721Enumerable(elfNFTContractAddress).tokenOfOwnerByIndex(_userAddress, index)) ;
        }
